"""
Negative control: fixed upstream exon (shuffled) vs variable exon.

Same plots as rnaduplex_upstream_exon_analysis.py but using the
mononucleotide-shuffled upstream exon sequence.
"""
...
duplex = pd.read_csv(BASE / "data/test_rnaduplex_upstream_exon_shuffled.csv")
...
col  = "mfe_upstream_exon_shuffled"
...
jitter_boxplot(
    ...
    ylabel="Residual PSI",
    title=(f"Shuffled upstream exon vs variable exon — residual PSI\n"
           f"mean bot={bot['signed_residual'].mean():+.4f}, "
           f"rest={rest['signed_residual'].mean():+.4f}, "
           f"p={p_signed:.2e}"),
)
ax1.axhline(0, color="black", linewidth=0.8, linestyle="--", alpha=0.5)
fig1.suptitle("RNAduplex — SHUFFLED upstream exon vs variable exon\n"
              "mean residual PSI: bottom 2% MFE vs rest",
              fontsize=10, y=1.02)
fig1.tight_layout()
p1 = OUT / "rnaduplex_upstream_exon_shuffled_signed_residual.png"
...
ax2.set_title("Shuffled upstream exon vs variable exon\nresidual PSI variance (Levene)", fontsize=9)
ax2.set_ylim(0, ymax * 1.25)
fig2.suptitle("RNAduplex — SHUFFLED upstream exon vs variable exon\n"
              "residual PSI variance: bottom 2% MFE vs rest",
              fontsize=10, y=1.02)
fig2.tight_layout()
p2 = OUT / "rnaduplex_upstream_exon_shuffled_psi_variance.png"
