"""
Negative control: exon-intron interaction analysis with SHUFFLED intron chunks.

Same plots as rnaduplex_interact_analysis.py but using
data/test_rnaduplex_interact_shuffled.csv — intron chunks are
mononucleotide-shuffled, so any surviving signal is GC-content-driven.
"""
...
duplex  = pd.read_csv(BASE / "data/test_rnaduplex_interact_shuffled.csv")
...
for chunk in CHUNKS:
    ...
    print(f"{chunk:<10} {thr:>10.1f} {len(bot):>7}  "
...
for ax, r in zip(axes1, rows):
    ...
    jitter_boxplot(
        ...
        ylabel="Residual PSI",
        title=(f"{r['chunk']} [SHUFFLED] — residual PSI\n"
               f"mean bot={r['bot']['signed_residual'].mean():+.4f}, "
               f"rest={r['rest']['signed_residual'].mean():+.4f}, "
               f"p={r['p_signed']:.2e}"),
    )
    ax.axhline(0, color="black", linewidth=0.8, linestyle="--", alpha=0.5)
fig1.suptitle("RNAduplex shuffled intron-exon comparison — mean residual PSI: bottom 2% vs rest",
              fontsize=10, y=1.02)
fig1.tight_layout()
p1 = OUT / "rnaduplex_interact_shuffled_signed_residual.png"
...
for ax, r in zip(axes2, rows):
    ...
    ax.set_title(f"{r['chunk']} [SHUFFLED] — residual PSI variance (Levene)", fontsize=9)
    ax.set_ylim(0, ymax * 1.25)
fig2.suptitle("RNAduplex shuffled intron-exon comparison — residual PSI variance: bottom 2% vs rest",
              fontsize=10, y=1.02)
fig2.tight_layout()
p2 = OUT / "rnaduplex_interact_shuffled_psi_variance.png"
